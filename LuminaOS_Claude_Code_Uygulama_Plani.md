...
- F0-T1: pnpm workspace + Turborepo kurulumu, paket iskeletleri. *(Kabul: `pnpm build` tüm paketlerde yeşil)*
...
- F0-T4: `.claude/` yönlendirme yığınının kurulması (CLAUDE.md, 6 subagent, 5 skill, hook'lar). *(Kabul: örnek görevde ritüel uçtan uca çalışır)*
...
- *(Kabul: property-based testlerle şema evrimi ve ilişki tutarlılığı kanıtlı; %95+ kapsam)*
...
1. **Context Fabric (E):** Event sourcing + grafik projeksiyonu. Anahtar karar: bağlam *türetilir*, ayrıca yazılmaz — tek doğruluk kaynağı olay günlüğüdür.
