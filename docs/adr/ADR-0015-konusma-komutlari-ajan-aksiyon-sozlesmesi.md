...
**Karar (PR5 uygulamasından sonra AYRICA düzeltildi — iki farklı endişe birbirinden ayrıştırıldı):** İlk yazımda bu bölüm, F1-T16'nın yürütme katmanının `TaskRecurrenceService`'in Katman B desenini (`deterministicUuid()` ile stream/event id türetimi) birebir tekrarlayacağını öngörmüştü. PR5'in implementasyonu ve güvenlik incelemesi, İKİ AYRI endişenin ("aynı onay iki kez işlenirse ne olur" ve "gerçek mutasyon hangi onaya bağlı, event log'dan okunabilir mi") tek bir mekanizmaya (deterministik id) bağlanmasının gereksiz olduğunu ortaya çıkardı — bunlar bağımsız olarak, daha basit araçlarla çözülebilir:

- **İdempotency/çift-yürütme koruması TAMAMEN `command_proposals.decided_at` kontrolünden gelir** — `decide()`, event store'a veya yürütmeye dokunmadan ÖNCE bu kolonu kontrol eder (§d'nin kendi metninde zaten belirtildiği gibi) ve zaten karar verilmiş bir öneri için `ConflictError` fırlatır. Eşzamanlı (concurrent) iki `decide()` çağrısı için de güvenlidir: `ActionsDecided`'ın `eventStore.append`'i sabit `expectedVersion`'da yapılır, yarışı kaybeden çağrı bu adımda optimistic-concurrency çakışmasıyla başarısız olur — KENDİ yürütme döngüsüne hiç girmeden. Güvenlik incelemesi bu mekanizmayı hem ardışık hem eşzamanlı senaryolar için doğruladı. **`deterministicUuid()` tabanlı stream/event id türetimi KULLANILMIYOR** — gerekli değil, çünkü tekilliği zaten `decided_at` sağlıyor.
- **Denetlenebilirlik `causationEventId`'den gelir, ama DÜZ METADATA olarak** — `ActionsDecided` event'inin kendi id'si, yürütme sırasında üretilen gerçek mutasyon çağrılarına (`ObjectsService.create`/`RelationsService.create`) `causationEventId` alanı olarak AÇIKÇA geçirilir (herhangi bir deterministik türetim OLMADAN — sıradan bir UUID string'i gibi taşınır). Bu, `gerçek mutasyon ← ActionsDecided ← ActionsProposed` zincirini event log'un kendi payload içeriğinden okunabilir kılar (CLAUDE.md'nin "kaynaklar[]"/izlenebilirlik ruhu), ama tekillik/idempotency GARANTİSİ TAŞIMAZ — o sorumluluk tamamen `decided_at` kontrolündedir. `setFieldValues`/`FieldValueChanged` bu alanı almaz (paket seviyesinde bugün hiç desteklenmiyor, bu görev kapsamında yeni bir yetenek eklenmedi — yalnızca `createObject`/`createRelation`'ın ZATEN sahip olduğu opsiyonel alan kullanıldı).

Bu ayrıştırma, PR5'in implementasyonuyla birebir eşleşiyor ve güvenlik incelemesinden geçti.
...
- Hiçbir yeni idempotency/durum-makinesi altyapısı icat edilmiyor — idempotency `command_proposals.decided_at`'in tek, basit kontrolünden ve `EventStoreService`'in mevcut optimistic-concurrency'sinden (eşzamanlı `decide()` yarışını da kapsayacak şekilde) geliyor; denetlenebilirlik ise ayrıca, `causationEventId`'nin düz metadata olarak taşınmasından geliyor — iki farklı endişe, iki basit mekanizmaya ayrıştırılmış durumda (bkz. §b'nin PR5-sonrası düzeltmesi; ADR-0010'un tasarladığı DB-index katmanı hiç inşa edilmemiş, bu ADR o hataya dayanmıyor).
