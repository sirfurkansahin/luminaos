...
**Epik:** F1-E3 (Görev + Doküman + Takvim Çekirdeği) · **Durum:** Devam Ediyor (PR1 + PR2 + PR3 tamamlandı; wiring/hatırlatıcı/UI kalan işler)
...
- [ ] `status` alanı `isDone=true` seçeneğine geçince tam olarak bir yeni yinelenen görev üretildiği, önceki göreve `recurrence-of` ilişkisiyle bağlandığı testli; aynı tamamlanma olayının iki kez işlenmesi ikinci nesneyi üretmediği (idempotency) testli. Üretim mekanizmasının kendisi (`TaskRecurrenceService.generateNextOccurrence`) PR3'te (#38) ADR-0010'a göre uygulandı ve idempotency (sıralı + eşzamanlı) testlerle kanıtlandı; `status`→`isDone` geçiş TESPİTİ ve `ObjectsService`'e wiring'i henüz yapılmadı (ayrı bir sonraki PR), bu yüzden kutu kapatılmadı.
...
- **Kalan:** madde 2'nin property-based test kapsamı; madde 4'ün geri kalanı — `task`'a `recurrenceRule` alanı, `status`→`isDone` geçiş TESPİTİ, `ObjectsService.setFieldValues`'in `TaskRecurrenceService`'i çağırması (wiring), `relations_view`'a `causation_event_id` kolonu + partial unique index (ADR-0010 §c Layer A — DB migration, henüz yapılmadı); madde 5 (hatırlatıcı); madde 6 (UI widget'ları); Board regresyon testi.
